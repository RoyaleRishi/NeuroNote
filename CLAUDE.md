...
- **Mobile / responsive ("focus mode")** — Notion-inspired phone layout, kept on a two-step scale: **`≤768px`** (mobile layout) and **`≤430px`** (small-phone tightening). Do not add new workspace breakpoints outside this scale. At `≤768px` the workspace collapses to a single column and the note list becomes a left **slide-over drawer** (`.notes-sidebar.mobile-open` + `.mobile-sidebar-scrim`); it opens via the nav hamburger **and** a left-edge swipe (≤24px start zone, in `NotesWorkspace.tsx`), and closes on scrim tap, Escape, note-selection, **and note creation** (all routed through `setMobileSidebarOpen(false)`; Escape/outside-click via the shared `useDismissable` hook). The 3-card `.workspace-stat-grid` is replaced by a single mono `.workspace-stat-summary` line on mobile (grid `display:none`; the summary is the AT-visible source of counts there, so it must **not** be `aria-hidden`). Editor goes full-width with `--text-md` (17px) body type; the graph filter rail docks as a bottom sheet (canvas `order:-1`). Mobile layout tokens live in `tokens.css`: `--nav-height-mobile`, `--drawer-width` / `--drawer-width-sm`, `--touch-target-min` (44px) / `--touch-target-lg` (48px), and `--safe-top/bottom/left/right` (`env(safe-area-inset-*)`). Editor picker rules are scoped under `.notes-editor-panel`; OAuth button sizing is scoped to `.oauth-buttons` (both render branches in `OAuthButtons.tsx`) so the login footer link isn't inflated.
