...
-------------------------
Every public function takes a ``Session`` as its first argument. The
caller is responsible for binding that session to the correct tenant
schema (via ``bind_session_to_tenant``) before calling. The helpers
do not open ad-hoc sessions of their own — that previously bypassed
tenant context and caused ``processing_jobs`` writes to land in the
``public`` schema.
"""
...
def _get_session():  # type: ignore[return]
    ...
    Only used by ``mark_stale_jobs_as_failed`` (the startup loop) and
    legacy in-memory init paths. Public job_store helpers must use the
    caller's session — see module docstring.
    """
...
def mark_stale_jobs_as_failed() -> None:
    ...
    ``processing_jobs`` lives in each tenant schema. We open one session
    per tenant and bind it explicitly via ``bind_session_to_tenant`` —
    no reliance on the ContextVar fallback. Called once on startup;
    safe before users exist (catches DB error).
    """
    if not _is_postgres():
        return
    try:
        from app.db.engine import bind_session_to_tenant

        with _get_session() as session:
            rows = session.execute(
                text("SELECT schema_name FROM public.users")
            ).all()
...
def _pg_create_or_get_job(
    session: "Session", *, note_id: str, content_hash: str
) -> tuple[ProcessStatusResponse, bool]:
...
def _pg_get_job(session: "Session", job_id: str) -> ProcessStatusResponse | None:
    row = session.execute(
        text(
            """
            SELECT job_id, status, error, created_at, updated_at, extraction_summary
...
def create_or_get_job(
    ...
    """Insert (or return existing) processing job for ``(note_id, content_hash)``.

    The session must be bound to the correct tenant schema before calling.
    """
    if _is_postgres():
        return _pg_create_or_get_job(session, note_id=note_id, content_hash=content_hash)
...
def mark_job_running(session: "Session", job_id: str) -> ProcessStatusResponse | None:
    return _transition_job(session, job_id=job_id, status="running", error=None)
...
def mark_job_completed(
    ...
    *,
    ...
    return _transition_job(
        session, job_id=job_id, status="completed", error=None, extraction_summary=extraction_summary,
    )


def mark_job_failed(session: "Session", job_id: str, *, error: str) -> ProcessStatusResponse | None:
    return _transition_job(session, job_id=job_id, status="failed", error=error)


def get_job(session: "Session", job_id: str) -> ProcessStatusResponse | None:
    if _is_postgres():
        return _pg_get_job(session, job_id)
